...
contract FundMeTest is Test {
    ...
    function testFundFailsWithoutEnoughETH() public{
        vm.expectRevert(); //hey, the next line, should revert (not the vm. lines)
        // assert(This tx fails/reverts)
        fundMe.fund(); // send 0 value
    }

    function testFundUpdatesFundedDataStructure() public{
        vm.prank(USER); //The next TX will be sentby USER
    ...
    function testWithrawFromMultipleFundersCheaper() public funded{
        ...
        for(uint160 i = startingFunderIndex; i < numberOfFunders; i++){
            ...
            //vm.deal new address
            //address()
            hoax(address(i), SEND_VALUE); // it is a new address and funded
            fundMe.fund{value: SEND_VALUE}();
